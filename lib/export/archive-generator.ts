...
import { generateEvidenceCSV, generateDatasetsCSV, generateProtocolsCSV } from './csv-generator'
  ...
  return new Promise((resolve, reject) => {
    ...
    const protocolPromises = protocols.map(async (protocol) => {
      try {
        ...
        const safeName = protocol.title.replace(/[^a-z0-9]/gi, '-').toLowerCase()
        archive.append(pdfBuffer, {
          name: `protocols/${safeName}-${protocol.id.substring(0, 8)}.pdf`,
        })
      } catch (error) {
        console.error(`Error generating PDF for protocol ${protocol.id}:`, error)
      }
    })
...
You can import the JSON file into another VaxEvidence instance or use the CSV files for analysis.
`
