...
export async function generateProtocolPDF(
  ...
  addText(protocol.intervention || "Not specified", 11, false);
  yPosition += 3;
    ...
    linkedEvidence.forEach((link, index) => {
      ...
      if (evidence.type === "academic") {
        ...
        if (evidence.regulatory_body) {
          doc.text(
            `   Regulatory Body: ${evidence.regulatory_body}`,
            marginLeft,
            yPosition,
          );
          yPosition += 4;
  ...
  if (options.includeDatasets && linkedDatasets.length > 0) {
    ...
    linkedDatasets.forEach((link, index) => {
      ...
      if (dataset.row_count) {
        doc.text(
          `   Rows: ${dataset.row_count.toLocaleString()} × ${dataset.column_count} columns`,
          marginLeft,
          yPosition,
        );
        yPosition += 4;
  ...
  if (options.includeComments && comments.length > 0) {
    ...
    comments.forEach((comment, index) => {
      ...
      doc.setFont("helvetica", "bold");
      doc.text(
        `${comment.user?.email || "Unknown User"} - ${format(new Date(comment.created_at), "MMM d, yyyy HH:mm")}`,
        marginLeft,
        yPosition,
      );
      yPosition += 5;
  ...
  if (options.includeReviews && reviews.length > 0) {
    ...
    reviews.forEach((review) => {
      ...
      doc.text(
        `${reviewerEmail} - ${review.status.toUpperCase()}`,
        marginLeft,
        yPosition,
      );
      yPosition += 5;
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        `Requested: ${format(new Date(review.requested_at), "MMM d, yyyy")}`,
        marginLeft + 3,
        yPosition,
      );
      yPosition += 4;
...
export async function generateAuditLogPDF(
  ...
  activityLogs.forEach((log, index) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      `${index + 1}. ${format(new Date(log.created_at), "MMM d, yyyy HH:mm:ss")}`,
      marginLeft,
      yPosition,
    );
    yPosition += 5;
