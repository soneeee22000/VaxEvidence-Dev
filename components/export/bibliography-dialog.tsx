...
export function BibliographyDialog({
  ...
  const [format, setFormat] = useState<'bibtex' | 'apa' | 'mla' | 'chicago' | 'ris'>('apa')
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast({
        title: 'Export failed',
        description: error instanceof Error ? error.message : 'Failed to generate bibliography',
        variant: 'destructive',
      })
    } finally {
        ...
        <div className="space-y-4 py-4">
          ...
          <RadioGroup value={format} onValueChange={(value: any) => setFormat(value)}>
            <div className="flex items-center space-x-2">
              ...
              <Label htmlFor="format-apa" className="font-normal cursor-pointer">
                APA (American Psychological Association)
              </Label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="mla" id="format-mla" />
              <Label htmlFor="format-mla" className="font-normal cursor-pointer">
                MLA (Modern Language Association)
              </Label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="chicago" id="format-chicago" />
              <Label htmlFor="format-chicago" className="font-normal cursor-pointer">
                Chicago (Harvard Style)
              </Label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="bibtex" id="format-bibtex" />
              <Label htmlFor="format-bibtex" className="font-normal cursor-pointer">
                BibTeX (for LaTeX)
              </Label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="ris" id="format-ris" />
              <Label htmlFor="format-ris" className="font-normal cursor-pointer">
          ...
          <p className="text-xs text-muted-foreground mt-4">
            Only academic evidence items with authors will be included in the bibliography.
          </p>
        </div>
