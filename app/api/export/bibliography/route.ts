...
import { generateBibliography, generateRegulatoryDocumentList } from '@/lib/export/bibliography'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!format || !['bibtex', 'apa', 'mla', 'chicago', 'ris'].includes(format)) {
      return NextResponse.json(
        { error: 'Invalid format. Must be one of: bibtex, apa, mla, chicago, ris' },
        { status: 400 }
      )
    }

    // Fetch linked evidence
    const { data: linkedEvidence, error } = await getLinkedEvidence(protocolId)
    ...
    const bibliography = generateBibliography(linkedEvidence, format as BibliographyFormat)
    ...
    return NextResponse.json(
      { error: 'Failed to generate bibliography', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
