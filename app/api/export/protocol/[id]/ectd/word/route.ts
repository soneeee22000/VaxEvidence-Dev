...
import { generateECTDWord } from "@/lib/export/ectd-word-generator";
import type { ECTDPackageData } from "@/lib/export/ectd-pdf-generator";

/**
 * Export protocol as eCTD Module 5 Clinical Study Report (Word)
 * POST /api/export/protocol/[id]/ectd/word
 */
export async function POST(
  ...
  try {
    ...
    // Generate eCTD Module 5 Word document
    const wordBlob = await generateECTDWord(packageData);
    ...
    return new Response(wordBlob, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="eCTD-Module5-${safeTitle}-${protocolId.substring(0, 8)}.docx"`,
        "Cache-Control": "no-cache",
      },
    });
  } catch (error) {
    console.error("Error generating eCTD Module 5 Word:", error);
    const isDev = process.env.NODE_ENV === "development";
    return NextResponse.json(
      {
        error: "Failed to generate eCTD Module 5 Word document",
        ...(isDev && {
