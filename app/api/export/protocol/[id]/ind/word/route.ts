...
/**
 * Export protocol as FDA IND Package (Word)
...
export async function POST(
  ...
  try {
    ...
    // Generate IND Word document
    const wordBlob = await generateINDPackageWord(packageData);
    ...
    return new Response(wordBlob, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="IND-${safeTitle}-${protocolId.substring(0, 8)}.docx"`,
        "Cache-Control": "no-cache",
      },
    });
  } catch (error) {
    console.error("Error generating IND package Word:", error);
    const isDev = process.env.NODE_ENV === "development";
    return NextResponse.json(
      {
        error: "Failed to generate IND package Word document",
        ...(isDev && {
