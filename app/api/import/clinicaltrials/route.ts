...
export async function POST(request: NextRequest) {
  ...
  if (!body?.nctId || !body?.title) {
    return NextResponse.json({ error: "NCT ID and title required" }, { status: 400 })
  }
  ...
  try {
    ...
    const payload = {
      ...
      document_type: [body.phase, body.status].filter(Boolean).join(" • ") || null,
    ...
    if (error || !data) {
      return NextResponse.json({ error: "Failed to import trial" }, { status: 500 })
    }
